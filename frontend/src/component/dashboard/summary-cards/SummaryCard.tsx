  ...
  return (
    <Card className={`${styles.glassmorphicCard("primary")} h-full group`}>
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
        <CardTitle
          className={`text-sm font-medium ${styles.text("primary")} group-hover:text-white/100 dark:group-hover:text-orange-50 transition-colors duration-300`}
        >
          {title}
        </CardTitle>
      </CardHeader>
      <CardContent className="relative z-10">
        <div
          className={`text-2xl font-bold drop-shadow-sm mb-2 ${styles.text("primary")} group-hover:text-white/100 dark:group-hover:text-orange-100 transition-colors duration-300`}
        >
          {value}
        </div>
        <p
