  ...
  return (
    <Card className={`${styles.glassmorphicCard('primary')} h-full group`}>
      ...
      <CardHeader className="relative z-10">
        <CardTitle className={`text-lg drop-shadow-sm ${styles.text('primary')} group-hover:text-white/100 dark:group-hover:text-orange-50 transition-colors duration-300`}>
          Your URLs
        </CardTitle>
        <CardDescription className={`${styles.text('muted')} group-hover:text-white/90 dark:group-hover:text-orange-200/80 transition-colors duration-300`}>
      ...
      <CardContent className="relative z-10">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/20 dark:border-orange-400/10">
                  <th className={`text-left p-2 font-medium ${styles.text('secondary')}`}>
                    URL
                  </th>
                  <th className={`text-left p-2 font-medium ${styles.text('secondary')}`}>
                    Destination
              ...
              <tbody>
                {paginatedUrls.map((url) => (
                  ...
                  >
                    ...
                    <td className={`p-2 drop-shadow-sm ${styles.text('primary')} group-hover:text-white/100 dark:group-hover:text-orange-100 transition-colors duration-300`}>
                      {url.clicks}
                    </td>
                    <td className={`p-2 ${styles.text('secondary')} group-hover:text-white/100 dark:group-hover:text-orange-200 transition-colors duration-300`}>{url.ttl}</td>
                    <td className="p-2">
                      {url.status === "Active" ? (
                    ...
                    <td className="p-2">
                      {url.protected ? (
                        <Shield className={`h-4 w-4 drop-shadow-sm ${styles.text('primary')} group-hover:text-white/100 dark:group-hover:text-orange-200 transition-colors duration-300`} />
                      ) : (
                        <span className={`${styles.text('muted')} group-hover:text-white/100 dark:group-hover:text-orange-300 transition-colors duration-300`}>—</span>
                      )}
                    </td>
                    <td className={`p-2 ${styles.text('secondary')} group-hover:text-white/100 dark:group-hover:text-orange-200 transition-colors duration-300`}>{url.createdAt}</td>
                    <td className="p-2">
        ...
        {urls.length > itemsPerPage && (
          <div className="flex justify-center gap-2 mt-4">
            ...
            <span className={`flex items-center px-4 text-sm ${styles.text('muted')}`}>
