...
export const useThemeStyles = () => {
  const { theme } = useTheme();  const getGlassmorphicCardStyles = (variant: 'primary' | 'secondary' | 'accent' = 'primary') => {
    const baseStyles = "backdrop-blur-xl transition-all duration-500 relative overflow-hidden group";
      if (theme === 'dark') {
      switch (variant) {
        case 'primary':
  ...
  const getTextStyles = (variant: 'primary' | 'secondary' | 'muted' = 'primary') => {
    if (theme === 'dark') {
      switch (variant) {
        case 'primary':
